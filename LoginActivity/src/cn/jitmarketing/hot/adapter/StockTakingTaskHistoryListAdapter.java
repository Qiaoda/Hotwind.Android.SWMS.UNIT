...
import cn.jitmarketing.hot.pandian.StockTakingShopperHistoryActivity;
...
public class StockTakingTaskHistoryListAdapter extends BaseAdapter {

	private List<StockTakingTaskEntity> stockList;
	private LayoutInflater inflater;    
	private Activity activity;

	public StockTakingTaskHistoryListAdapter(Activity activity, List<StockTakingTaskEntity> stockList) {
...
//					activity.startActivity(intent);
				intent.setClass(activity, StockTakingShopperHistoryActivity.class);
				Bundle bd = new Bundle();
				bd.putString("checkTaskID", task.CheckTaskID);
				bd.putString("checkTaskName", task.CheckTaskName);
				intent.putExtras(bd);
				activity.startActivity(intent);
				//日志操作
				LogUtils.logOnFile("任务盘点->历史盘点->盘点单号："+task.CheckTaskName);
			}
		});
		return convertView;
	}
